...
char *msg_toString(const HvMessage *m) {
  ...
  for (int i = 0; i < msg_getNumElements(m); i++) {
    ...
    switch (msg_getType(m, i)) {
      ...
      case HV_MSG_HASH: len[i] = hv_snprintf(NULL, 0, "0x%X", (unsigned int) msg_getHash(m, i)) + 1; break;
      default: break;
    }
    size += len[i];
  }
  ...
  for (int i = 0; i < msg_getNumElements(m); i++) {
    ...
    switch (msg_getType(m, i)) {
      ...
      case HV_MSG_HASH: hv_snprintf(finalString+pos, len[i], "0x%X", (unsigned int) msg_getHash(m, i)); break;
      default: break;
    }
    pos += len[i];
